...
def convert(xml_list, json_file):
    ...
    for index, line in enumerate(xml_list):
        ...
        xml_f = line
        if xml_f[-4:] not in ['.xml'] or os.path.basename(xml_f) in ['115.xml']:
            continue
        print(xml_f)
        tree = ET.parse(xml_f)
        root = tree.getroot()
...
if __name__ == '__main__':
    ...
    pre_define_categories = {'car': 1, 'bus' : 2, 'truck':3, 'person':4, 'motorcycle':5}
    only_care_pre_define_categories = True  # or False

    # 保存的json文件
    save_json_train = 'train_road.json'
    # save_json_val = 'val_food.json'
    save_json_test = 'test_road.json'

    # 初始文件所在的路径
    xml_dir = "/Users/videopls/Desktop/own_dataset/orign_ann/"
    xml_train_list = glob.glob(xml_dir + "train/*.xml")
    xml_test_list = glob.glob(xml_dir + 'test/*.xml')
    ...
    convert(xml_train_list, save_json_train)
    # convert(xml_list_val, save_json_val)
    convert(xml_test_list, save_json_test)
    ...
    print("-" * 50)
    print("train number:", len(xml_train_list))
    # print("val number:", len(xml_list_val))
    print("test number:", len(xml_test_list))
